...
class ToyNetValueEntryById(Resource):
    ...
    def get(self, value_id):
        ...
        try:
            rows = db.execute(
                'SELECT v.id, ve.quote, strftime(\'%Y-%m-%d %H:%M:%S::%s:%f\', ve.created) as created'
    ...
    def put(self, value_id):
        ...
        try:
            db.execute(
                'REPLACE INTO toynet_value_entries(value_id, username, user_group_id, quote, created)'
